"""
Multi-layer Recurrent Neural Networks (LSTM, RNN).
...
* https://github.com/ITPNYU/ml5/tree/master/training/lstm

This script will train and dump the checkpoints to js
"""
...
from port_weights import dump_checkpoints
...
def train(args):
    ...
    if args.init_from is not None:
        ...
        assert os.path.isdir(args.init_from), \
            "{} must be a a path".format(args.init_from)
        ...
        assert os.path.isfile(os.path.join(args.init_from, "chars_vocab.pkl")), \
            "chars_vocab.pkl.pkl file does not exist in path {}".format(args.init_from)
        ...
        for checkme in need_be_same:
            assert vars(saved_model_args)[checkme] == vars(args)[checkme], \
                "Command line argument and saved model disagree on '{}' ".format(checkme)

        # open saved vocab/dict and check if vocabs/dicts are compatible
        with open(os.path.join(args.init_from, 'chars_vocab.pkl'), 'rb') as f:
            saved_chars, saved_vocab = cPickle.load(f)

        assert saved_chars == data_loader.chars, \
            ...
            for b in range(data_loader.num_batches):
                ...
                if (e * data_loader.num_batches + b) % args.save_every == 0 or \
                   (e == args.num_epochs - 1 and b == data_loader.num_batches - 1):
                    ...
                    current_checkpoints = [
                        cp for cp in os.listdir(args.save_dir)
                        if os.path.isfile(os.path.join(args.save_dir, cp))
                    ]
